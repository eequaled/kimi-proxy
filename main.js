...
async function handleChatCompletion(req, res, payload) {
  ...
  const upReq = https.request(options, (upRes) => {
    ...
    if (stream) {
      ...
      processStreamResponse(upRes, res, requestId, created, modelId, hasTools);
    } else {
      processBufferResponse(upRes, res, requestId, created, modelId, hasTools);
    }
  });
...
async function processStreamResponse(upRes, clientRes, requestId, created, model, hasTools) {
  ...
  // Process buffer for tool call markers (state machine)
  const processBuffer = () => {
    while (buffer) {
      if (inToolCall) {
        ...
        if (endIdx !== -1) {
          ...
          try {
            ...
            else streamContent(START_TAG + buffer.slice(0, endIdx + END_TAG.length));
          } catch (e) {
            streamContent(START_TAG + buffer.slice(0, endIdx + END_TAG.length));
          }
        ...
        if (startIdx !== -1) {
          streamContent(buffer.slice(0, startIdx));
          ...
          if (safeLen > 0) {
            streamContent(buffer.slice(0, safeLen));
    ...
    for await (const frame of parseConnectStream(upRes)) {
      ...
      if (payload.op === "append" && payload.mask === "block.text.content" && payload.block?.text?.content) {
        if (hasTools) {
          ...
          streamContent(payload.block.text.content);
        }
  ...
  if (buffer) {
    if (inToolCall) {
      streamContent(START_TAG + buffer);
    } else {
      streamContent(buffer);
    }
  }
...
async function processBufferResponse(upRes, clientRes, requestId, created, model, hasTools) {
  let textContent = "";
  let thinkContent = "";
    ...
    if (hasTools && textContent) {
      ...
      if (calls.length > 0) {
        ...
        calls.forEach((call, i) => {
          ...
          toolCalls[i].function.arguments = typeof call.arguments === "string"
            ? call.arguments
            : JSON.stringify(call.arguments);
        });
